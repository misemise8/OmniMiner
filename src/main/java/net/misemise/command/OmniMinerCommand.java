...
public class OmniMinerCommand {
                        ...
                        .then(CommandManager.literal("set")
                                .then(CommandManager.literal("maxBlocks")
                                        .then(CommandManager.argument("value", IntegerArgumentType.integer(1, 512))
                                                .executes(ctx -> setMaxBlocks(ctx, IntegerArgumentType.getInteger(ctx, "value")))
                                        )
                                )
                                .then(CommandManager.literal("searchDiagonal")
                                        .then(CommandManager.argument("value", BoolArgumentType.bool())
                                                .executes(ctx -> setSearchDiagonal(ctx, BoolArgumentType.getBool(ctx, "value")))
                                        )
                                )
                                .then(CommandManager.literal("autoCollect")
                                        .then(CommandManager.argument("value", BoolArgumentType.bool())
                                                .executes(ctx -> setAutoCollect(ctx, BoolArgumentType.getBool(ctx, "value")))
                                        )
                                )
                                .then(CommandManager.literal("autoCollectExp")
                                        .then(CommandManager.argument("value", BoolArgumentType.bool())
                                                .executes(ctx -> setAutoCollectExp(ctx, BoolArgumentType.getBool(ctx, "value")))
                                        )
                                )
                                .then(CommandManager.literal("breakLeaves")
                                        .then(CommandManager.argument("value", BoolArgumentType.bool())
                                                .executes(ctx -> setBreakLeaves(ctx, BoolArgumentType.getBool(ctx, "value")))
                                        )
                                )
                                .then(CommandManager.literal("toggleMode")
                                        .then(CommandManager.argument("value", BoolArgumentType.bool())
                                                .executes(ctx -> setToggleMode(ctx, BoolArgumentType.getBool(ctx, "value")))
                                        )
                                )
                                .then(CommandManager.literal("bedrockSneakEnable")
                                        .then(CommandManager.argument("value", BoolArgumentType.bool())
                                                .executes(ctx -> setBedrockSneakEnable(ctx, BoolArgumentType.getBool(ctx, "value")))
                                        )
                                )
                                .then(CommandManager.literal("bedrockShowParticles")
                                        .then(CommandManager.argument("value", BoolArgumentType.bool())
                                                .executes(ctx -> setBedrockShowParticles(ctx, BoolArgumentType.getBool(ctx, "value")))
                                        )
                                )
                                .then(CommandManager.literal("bedrockParticleMode")
                                        .then(CommandManager.argument("value", IntegerArgumentType.integer(0, 1))
                                                .executes(ctx -> setBedrockParticleMode(ctx, IntegerArgumentType.getInteger(ctx, "value")))
                                        )
                                )
                        )
    ...
    private static int showConfig(CommandContext<ServerCommandSource> ctx) {
        ...
        if (floodgateAvailable) {
            source.sendFeedback(() -> Text.literal("§eBedrock Sneak Enable: §f" + bedrockSneakEnable), false);
            source.sendFeedback(() -> Text.literal("§eBedrock Allow KeyBind: §f" + bedrockAllowKeyBind), false);
        }
    ...
    private static int showHelp(CommandContext<ServerCommandSource> ctx) {
        ...
        if (floodgateAvailable) {
            source.sendFeedback(() -> Text.literal("§e/omniminer set bedrockSneakEnable <true|false>"), false);
        }
    ...
    private static int setBedrockSneakEnable(CommandContext<ServerCommandSource> ctx, boolean value) {
        ...
        Config.save();
        final boolean finalValue = value;
        ctx.getSource().sendFeedback(() -> Text.literal("§aBedrock Sneak Enable set to: §f" + finalValue), false);
        return 1;
    }
    ...
    private static int setBedrockShowParticles(CommandContext<ServerCommandSource> ctx, boolean value) {
        ...
        Config.save();
        final boolean finalValue = value;
        ctx.getSource().sendFeedback(() -> Text.literal("§aBedrock Show Particles set to: §f" + finalValue), false);
        return 1;
    }
    ...
    private static int setBedrockParticleMode(CommandContext<ServerCommandSource> ctx, int value) {
        ...
        ctx.getSource().sendFeedback(() -> Text.literal("§aBedrock Particle Mode set to: §f" + finalValue + " (" + modeName + ")"), false);
        return 1;
    }
}