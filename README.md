...
現時点では、実際にビルドと起動確認を行った Minecraft 26.1.2 のみを
メタデータ上でも許可しています。
...
- 工具耐久は、バニラと同じく実際に壊したブロックごとに消費されます。
- 一 tick に処理する追加ブロックは最大 32 個です。
- 統合版プレビューは、一回の更新につき最大 128 ブロック分を順番に表示します。
...
JSON が壊れている場合は日時付きの `omniminer.invalid-*.json` へ退避し、既定設定を再生成します。
